...
__all__ = ["Decoder", "my_dynamic_decode"]
...
def my_dynamic_decode(decoder,
  ...
  with variable_scope.variable_scope(scope, "decoder") as varscope:
    ...
    initial_outputs_ta = nest.map_structure(_create_ta, decoder.output_size,
                                            decoder.output_dtype)
    initial_states_ta = nest.map_structure(_create_ta, decoder.state_size,
                                           decoder.state_dtype)

    def condition(unused_time, unused_outputs_ta, unused_states_ta, unused_state, unused_inputs,
                  finished, unused_sequence_lengths):
      return math_ops.logical_not(math_ops.reduce_all(finished))

    def body(time, outputs_ta, states_ta, state, inputs, finished, sequence_lengths):
      ...
      outputs_ta = nest.map_structure(lambda ta, out: ta.write(time, out),
                                      outputs_ta, emit)
      states_ta = nest.map_structure(lambda ta, out: ta.write(time, out),
                                    states_ta, next_state)
      return (time + 1, outputs_ta, states_ta, next_state, next_inputs, next_finished,
              next_sequence_lengths)
    ...
    res = control_flow_ops.while_loop(
        ...
        body,
        loop_vars=[
            initial_time, initial_outputs_ta, initial_states_ta, initial_state, initial_inputs,
    ...
    final_states_ta = res[2]
    final_state = res[3]
    final_sequence_lengths = res[6]

    final_outputs = nest.map_structure(lambda ta: ta.stack(), final_outputs_ta)
    decoder_states = nest.map_structure(lambda ta: ta.stack(), final_states_ta)
    ...
    if not output_time_major:
      ...
      decoder_states = nest.map_structure(_transpose_batch_time, decoder_states)

  return final_outputs, decoder_states, final_state, final_sequence_lengths