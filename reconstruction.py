...
class BiRNN_seq2seq():
    def __init__(self, hparams, mode):
        ...
        with tf.variable_scope("reconstructor") as scope:
            ...
            if mode != tf.contrib.learn.ModeKeys.INFER:
                ...
                my_reconstructor = tf.contrib.seq2seq.BasicDecoder(cell=reconstructor_cell,
                                                             helper=helper,
                                                             initial_state=initial_state,
                ...
                self.logits = reconstructor_outputs.rnn_output
    ...
    def get_batch(self, data, buckets, bucket_id, batch_size):
        ...
        for _ in range(batch_size):
            ...
            encoder_pad_size = size - len(encoder_input)
            decoder_pad_size = size - len(decoder_input) - 1
            #print(len(encoder_input))
            encoder_inputs.append(encoder_input +
                                  [data_utils.PAD_ID] * encoder_pad_size)
            targets.append(decoder_input + [data_utils.EOS_ID] + [data_utils.PAD_ID] * decoder_pad_size)
            decoder_inputs.append([data_utils.GO_ID] + decoder_input
                                  + [data_utils.PAD_ID] * decoder_pad_size)
            target_weights.append([1.0] * (len(decoder_input) + 1) + [0.0] * decoder_pad_size)
            source_sequence_length.append(len(encoder_input))
            target_sequence_length.append(len(decoder_input) + 1)
